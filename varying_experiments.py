...
class environment(object):
    '''
    Environment is described by normalized svalues of training value (y-mu)/sd.
    '''
...
with open("model_varying_experiments_1.json", "w") as json_file:
    json_file.write(target_json)
# serialize weights to HDF5
target_model.save_weights("model_varying_experiments_1.h5")
print("Saved model to disk")

# load json and create model
json_file = open('model_varying_experiments_1.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
model_3 = tf.keras.models.model_from_json(loaded_model_json)

model_3.load_weights("model_varying_experiments_1.h5")
...
improvement = pd.DataFrame(improvement.astype('float'),columns = ['Increase_T_c']).to_csv('model_4_test.csv')
