...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 29 15:59:03 2021
...
json_file.close()
model_2 = tf.keras.models.model_from_json(loaded_model_json)
# load weights into new model
model_2.load_weights("target_model_positive.h5")
...
for episode in range(Test_runs):
    ...
    for i in range(number_experiments):
        encoded = encode_observation(observation, observation.shape[0])
        encoded_reshaped = encoded.reshape([1, encoded.shape[0]])
        predicted = model_2.predict(encoded_reshaped).flatten()
        action = np.argmax(predicted)
        new_observation, reward, done, info = env.step(action,i)
        observation = new_observation
        total_training_rewards += reward
...
improvement = pd.DataFrame(improvement.astype('float'),columns = ['Increase_T_c']).to_csv('model_2_test.csv')
